...
def setup_args():
    ...
    parser.add_argument('--exp_description', type=str, default='no-positional-encoding-with-motion-loss', help='Experiment description')
    ...
    parser.add_argument('--nca_padding_mode', type=str, default='replicate', help='Padding mode in perception')
    ...
    parser.add_argument('--auxillary_loss_type', type=str, default='MSE', help='Type of auxiliary loss')
...
def main():
    ...
    wandb.init(
        ...
        project='Video_Stylization_with_NCA', 
        entity='aliraed-benmustapha',
        config = {'experiment_index': experiment_index, **vars(args)}
        )
    wandb.run.name = args.exp_description
    ...
    try:
        for i in pbar:
            ...
            wandb.log({
                ...
                **batch_loss_log_dict,            # Expand the dictionary to log each component separately
            })
    ...
    ## Generate Videos
    ## CORGI
    save_video(f"experminets/experiment{experiment_index}/corgi", target_vid_path='data/Reference/corgi.gif', size_factor=2.0, step_n=int(args.nca_base_num_steps), steps_per_frame=10, is_style_image=False, nca_model=nca_model, nca_size_x=nca_size_x, nca_size_y=nca_size_y, DEVICE=DEVICE)
    ##GOT
    save_video(f"experminets/experiment{experiment_index}/got", target_vid_path='data/Reference/got.gif', size_factor=3.0, step_n=int(args.nca_base_num_steps), steps_per_frame=2, is_style_image=False, nca_model=nca_model, nca_size_x=nca_size_x, nca_size_y=nca_size_y, DEVICE=DEVICE)
    ##MR BEAN
    save_video(f"experminets/experiment{experiment_index}/mr_bean", target_vid_path='data/Reference/mr-bean.gif', size_factor=2.0, step_n=int(args.nca_base_num_steps), steps_per_frame=2, is_style_image=False, nca_model=nca_model, nca_size_x=nca_size_x, nca_size_y=nca_size_y, DEVICE=DEVICE)
    ##STARRY NIGHT
    save_video(f"experminets/experiment{experiment_index}/{args.style_name}", target_vid_path='data/Reference/{style_name}.gif', size_factor=2.0, step_n=int(args.nca_base_num_steps), steps_per_frame=2, is_style_image=False, nca_model=nca_model, nca_size_x=nca_size_x, nca_size_y=nca_size_y, DEVICE=DEVICE)
    # Raed Webcam
    save_video(f"experminets/experiment{experiment_index}/raed_webcam", target_vid_path='data/Reference/raed.mp4', size_factor=2.0, step_n=int(args.nca_base_num_steps), steps_per_frame=2, is_style_image=False, nca_model=nca_model, nca_size_x=nca_size_x, nca_size_y=nca_size_y, DEVICE=DEVICE)
    # Black background
    save_video(f"experminets/experiment{experiment_index}/black_background", target_vid_path='data/Reference/black_background.mp4', size_factor=2.0, step_n=int(args.nca_base_num_steps), steps_per_frame=2, is_style_image=False, nca_model=nca_model, nca_size_x=nca_size_x, nca_size_y=nca_size_y, DEVICE=DEVICE)
